...
const cluster = new src_1.Cluster({
    nodes: [
        {
...
gateway.on('MESSAGE_CREATE', async (shard, m) => {
    ...
    if (m.content === 'join')
        await cluster.get('281630801660215296').join('281630801660215297');
    if (m.content === 'leave')
        await cluster.get('281630801660215296').leave();
    if (m.content === 'pause')
        await cluster.get('281630801660215296').pause();
    if (m.content === 'destroy')
        await cluster.get('281630801660215296').destroy();
    if (m.content === 'decode') {
